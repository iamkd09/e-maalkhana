<?php

$en = array(
    // Login page keys
    "e"=>"E",
    "malkhana"=>"Maalkhana",
    "marching"=>"Towards Swachh Bharat",
    "smart"=>"Smart Policing and Marching",
    "project_heading"=>"'Smart Policing and Marching Towards Swacch Bharat'",
    "enter_mobile"=>"Enter Mobile Number",
    "otp_generate"=>"Enter Password",
    // Otp Page Keys
    "validate"=>"Login",
    "otp_sent"=>"4 digit OTP code has been sent to your registered mobile no.",
    "enter_otp"=>"Please enter password to verify your account",
    "not_got_otp"=>"Didn't get the code?",
    "resend_otp"=>"Resend it",
    "invalid_otp"=> "Invalid Password, please try again.",
    "nonactive_user" => "User is not active, please try again.",
    "invalid_user" => "Invalid User!.",
    "reset_button"=>"Reset",
    "new_password" => "New Password",

    // Dashbboard Page Keys
    "sidebar_home"=>"HOME",
    "sidebar_register"=>"REGISTER",
    "sidebar_userlist" => "USER LIST",
    "sidebar_languages"=>"LANGUAGES",
    "sidebar_inventory" => "INVENTORY",
    "sidebar_logout"=>"LOGOUT",
    "dashboard_search"=>"Search Inventory",
    "search_mobile"=>"Search Mobile Number",
    "inward_button"=>"Inward",
    "scrapyard_button"=>"Scrapyard",
    "auction_button"=>"Auction",
    "outward_button"=>"Outward",
    "scrap_auct_button"=>"Scrapyard/Auction",

    "inward_cases" => "Inward Items",
    "outward_cases" => "Outward Items",
    "scrapyard_cases" => "Scrapyard Items",
    "auction_cases" => "Auction Items",
    "from"=>"From",
    "to"=>"To",
    "last_one_month"=>"  (Last 1 Month Items)",

    // Search Page
    "go_button"=>"Go",
    "no_data"=>"No Data Found",

    // User Form
    "form"=>"form",
    "register_here"=>"Register Here",
    "station_name"=>"Police station name",
    "role_select"=>"Select Role",
    "mobile_number"=>"Mobile Number",
    "address"=>"Address",
    "state"=>"State",
    "select_state"=>"select state",
    "city"=>"City",
    "select_city"=>"select city",
    "role_name"=>"Role Name",
    "password"=>"Password",
    "submit"=>"Submit",

    "bhasha" => "Language",

    // Inward Form
    "inward_form"=>"Inward Form",
    "category"=>"Category",
    "sub_category"=>"Sub-category",
    "gd_number"=>"G.D. Number/F.I.R. Number",
    "stolen_date"=>"Stolen Date",
    "recovery_date"=>"Date of Seized",
    "fir_number"=>"",
    "under_section"=>"Under-Section",
    "recovered_from"=>"Seized From",
    "recovered_by"=>"Seized By",
    "accident_date"=>"Accident Date",
    "release_date"=>"Release Date",
    "vehicle_number"=>"Chasis Number/Engine Number",
    "vehicle_type"=>"Vehicle Type",
    "two_wheeler"=>"Two Wheeler",
    "three_wheeler"=>"Three Wheeler",
    "four_wheeler"=>"Four Wheeler",
    "vehicle_make"=>"Vehicle Make",
    "vehicle_model"=>"Vehicle Model",
    "vehicle_variant"=>"Vehicle Variant",
    "vehicle_color"=>"Vehicle Colour",
    "item_desc"=>"Item Description",
    "attachments"=>"Attachment of vehicle owner (Aadhaar , DL, RC, Insurance )",
    "pictures"=>"Pictures",
    "engine_number"=>"Engine Number",
    "chassis_number"=>"Chassis Number",
    "mv_act"=>"M.V. Act(Section)",
    "owner_name"=>"Name of Owner/Driver",
    "vehicle_r_number"=>"Vehicle Registration Number",
    "Unclaimed Vehicles"=>"Unclaimed vehicles form",
    "Accidental case"=>"Accidental case form",
    "Jama Talashi"=>"Jama talashi form",
    "MV Act"=>"M.V. Act. form",
    "Stolen Vehicles"=>"Stolen vehicles form",
    "Objects other than vehicles"=>"Objects other than vehicles form",
    "select_lang"=>"Select Language",
    "hindi"=>"Hindi",

    'sidebar_reset_password'=>'Change Password',
    'change_password'=>'Change',
    // Outward Form
    "outward_form"=>"Outward Form",
    "na"=> '',

    //Page Titles
    "outward.php" => 'Outward',
    "search.php" => 'Search',
    "dashboard.php" => 'Dashboard',
    "inward.php" => 'Inward',
    "scrapyard.php" => 'Scrapyard',
    "scrapyard_already.php" => 'Scrapyard',
    "auction_already.php" => 'Auction',
    "auction.php" => 'Auction',
    "user.php" => 'Register',
    "user_list.php" => 'User List',
    "language_change.php" => 'Translate',
    "inventory.php" => 'Inventory',
    "reset_config.php" => 'Reset Password',
    "change_password.php" => 'Change Password',
    "outward_list.php" => 'Outward',
    "outward_already.php" => 'Outward',
    "scrapped"=>"Eligible",
    "scrapped_already"=>"Already Present",

    // user_list
    "name" => "Name",
    "contact" => "Mobile No.",

    'cat_name' => 'Category',
    'sub_cat_name' => 'Sub-Category',

    'filter_button' => 'Filter',
    "reset_heading" => "Reset Password",
    "change_heading" => "Change Password",
    "scrapped_date"=>"Scrapped Date",
    "auctioned_date"=>"Auctioned Date",

);

$hi = array(
    // Login page keys
    "created"=>"निर्माण की तारीख",
    "e"=>"ई",
    "malkhana"=>"मालखाना",
    "marching"=>"की ओर अग्रसर",
    "smart"=>"“स्मार्ट पुलिसिंग और स्वच्छ भारत”",
    "enter_mobile"=>"मोबाइल नंबर दर्ज करें",
    "otp_generate"=>"पासवर्ड दर्ज करें",
    "password"=>"पासवर्ड",
    "reset_button"=>"रीसेट",
    // Otp Page Keys
    "validate"=>"लॉग इन करें",
    "otp_sent"=>"आपके रजिस्टर्ड मोबाइल नंबर पर 4 अंकों का ओटीपी भेजा गया है",
    "enter_otp"=>"अपना खाता सत्यापित करने के लिए कृपया ओटीपी दर्ज करें",
    "not_got_otp"=>"ओटीपी नहीं मिला?",
    "resend_otp"=>"फिर से भेजें",

    "invalid_otp"=> "अमान्य पासवर्ड, कृपया पुनः प्रयास करें।",
    "nonactive_user" => "उपयोगकर्ता सक्रिय नहीं है, कृपया पुनः प्रयास करें।",
    "invalid_user" => "अमान्य उपयोगकर्ता!।",
    "reset_heading" => "पासवर्ड रीसेट",
    "new_password" => "नया पासवर्ड",
    'change_password'=>'बदलें',
    "change_heading" => "पासवर्ड बदलें",

    // Dashbboard Page Keys
    "sidebar_home"=>"होम",
    "sidebar_register"=>"रजिस्टर करें",
    "sidebar_userlist" => "उपयोगकर्ता सूची",
    "sidebar_languages"=>"भाषा",
    "sidebar_inventory" => "वस्तुसूची",
    "sidebar_logout"=>"लॉग आउट",
    "dashboard_search"=>"सूची में खोजें",
    "search_mobile"=>"मोबाइल नंबर खोजें",
    "inward_button"=>"इन्वर्ड",
    "scrapyard_button"=>"रद्दीखाना",
    "auction_button"=>"नीलामी",
    "outward_button"=>"आउटवर्ड",
    "scrap_auct_button"=>"रद्दीखाना/नीलामी",

    'sidebar_reset_password'=>'पासवर्ड बदलें',

    "inward_cases" => "इन्वर्ड मामले",
    "outward_cases" => "आउटवर्ड मामले",
    "scrapyard_cases" => "रद्दीखाना मामले",
    "auction_cases" => "नीलामी मामले",

    "from"=>"शुरू तिथि",
    "to"=>"अंत तिथि",

    "last_one_month"=>" (पिछले 1 महीने के मामले)",
    // Search Page
    "go_button"=>"खोजे",
    "no_data"=>"डाटा प्राप्त नहीं हुआ",
    "register_here"=>"रजिस्टर करें",
    "station_name"=>"थाने का नाम",
    "role_select"=>"भूमिका चुनें",
    "mobile_number"=>"मोबाइल नंबर",
    "address"=>"पता",
    "state"=>"राज्य",
    "select_state"=>"राज्य चुनें",
    "city"=>"शहर",
    "select_city"=>"शहर चुनें",
    "submit"=>"जमा करें",

    "bhasha" => "भाषा",
    // Inward Form
    "form"=>"फॉर्म",
    "inward_form"=>"इन्वर्ड फॉर्म",
    "category"=>"श्रेणी",
    "sub_category"=>"उप-श्रेणी",
    "gd_number"=>"जनरल डायरी नंबर (जीडी नं)/एफ.आई.आर. संदर्भ संख्या",
    "stolen_date"=>"वाहन चोरी होने की तारीख",
    "recovery_date"=>"जब्त होने की तारीख",
    "fir_number"=>"एफ.आई.आर. संदर्भ संख्या",
    "under_section"=>"आईपीसी धारा के तहत",
    "recovered_from"=>"जब्त हुआ",
    "recovered_by"=>"के द्वारा जब्त",
    "accident_date"=>"दुर्घटना की तारीख",
    "release_date"=>"रिहाई की तारीख",
    "vehicle_number"=>"चेसिस नंबर/इंजन संख्या",
    "vehicle_type"=>"वाहन का प्रकार",
    "two_wheeler"=>"2 व्हीलर",
    "three_wheeler"=>"3 व्हीलर",
    "four_wheeler"=>"4 व्हीलर",
    "vehicle_make"=>"वाहन निर्माता",
    "vehicle_model"=>"वाहन का मॉडल",
    "vehicle_variant"=>"वाहन का संस्करण",
    "vehicle_color"=>"वाहन का रंग",
    "item_desc"=>"वस्तु विवरण",
    "attachments"=>"वाहन मालिक के प्रमाण के संलग्न (आधार, ड्राइविंग लाइसेंस, रजिस्ट्रेशन प्रमाणपत्र, बीमा)",
    "pictures"=>"तस्वीरें",
    "engine_number"=>"इंजन संख्या",
    "chassis_number"=>"चेसिस संख्या",
    "mv_act"=>"एम.वी. अधिनियम(धारा)",
    "owner_name"=>"मालिक का नाम",
    "vehicle_r_number"=>"वाहन पंजीकरण संख्या",
    "Unclaimed Vehicles"=>"लावारिस वाहन फॉर्म",
    "Jama Talashi"=>"जामा तलाशी फॉर्म",
    "Accidental case"=>"दुर्घटना मामला फॉर्म",
    "MV Act"=>"एम.वी. कार्यवाही फॉर्म",
    "Stolen Vehicles"=>"चोरी हुए वाहनों का फार्म",
    "Objects other than vehicles"=>"अन्य (वाहन के अलावा कुछ और वस्तु) का फार्म",
    "select_lang"=>"भाषा चुने",
    "hindi"=>"हिंदी",
    "role_name"=>"भूमिका",

    "outward_form"=>"आउटवर्ड फॉर्म",
    "na"=> '',

    //Page titles
    "outward.php" => 'आउटवर्ड',
    "search.php" => 'सर्च',
    "reset_config.php" => 'रीसेट पासवर्ड',
    "dashboard.php" => 'होम ',
    "inward.php" => 'इन्वर्ड',
    "scrapyard_already.php" => 'रद्दीखाना',
    "auction_already.php" => 'नीलामी',
    "scrapyard.php" => 'रद्दीखाना',
    "auction.php" => 'नीलामी',
    "user.php" => 'रजिस्टर',
    "language_change.php" => 'भाषा',
    "scrapped"=>"योग्य",
    "scrapped_already"=>"पहले से मौजूद",
    "inventory.php" => 'वस्तुसूची',
    "user_list.php" => 'उपयोगकर्ता सूची',
    "outward_list" => 'आउटवर्ड',
    "outward_already.php" => 'आउटवर्ड',
    "change_password.php" => 'पासवर्ड बदलें',
    'cat_name' => 'श्रेणी',
    'sub_cat_name' => 'उप-श्रेणी',

    'filter_button' => 'फ़िल्टर',

    
);

?>